...
# Kiểm tra sự tồn tại của file restore_svc.lst
if [ ! -f "${scrDir}/restore_svc.lst" ]; then
    print_log -sec "services" -crit "File restore_svc.lst not found in ${scrDir}"
    exit 1
fi
...
handle_legacy_service() {
    ...
    if systemctl list-units --all -t service --full --no-legend "${serviceChk}.service" | grep -q "${serviceChk}.service"; then
        ...
        if [ "$flg_DryRun" -ne 1 ]; then
            sudo systemctl enable "${serviceChk}.service" || {
                print_log -sec "services" -crit "Failed to enable ${serviceChk}.service"
                return 1
            }
        fi
    fi
...
# Main processing
print_log -sec "services" -stat "restore" "i3wm system services..."
        ...
        if [ "$flg_DryRun" -ne 1 ]; then
            if [ "$context" = "user" ]; then
                systemctl --user "${cmd_array[@]}" "${service}.service" || {
                    print_log -sec "services" -crit "Failed to execute systemctl --user ${cmd_array[*]} ${service}.service"
                    continue
                }
            else
                sudo systemctl "${cmd_array[@]}" "${service}.service" || {
                    print_log -sec "services" -crit "Failed to execute sudo systemctl ${cmd_array[*]} ${service}.service"
                    continue
                }
            fi
        else
...
print_log -sec "services" -stat "completed" "i3wm service updates"